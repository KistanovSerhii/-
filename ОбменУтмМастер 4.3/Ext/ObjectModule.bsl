...
Процедура СопоставитьАП(АП)
	...
	ГодУрожая = ПолучитьГодИзНаименования(АП.НаименованиеПолное);
	ОшибкаПолученияГодаУрожая = (ГодУрожая = Неопределено);
	
	Попытка
		ГодУрожаяАП = Число(ГодУрожая);
	Исключение
		ОшибкаПолученияГодаУрожая = Истина;
	КонецПопытки;

	Если ОшибкаПолученияГодаУрожая Тогда
		Сообщить("Отсутствует год урожая у АП с кодом " +  АП.Код, СтатусСообщения.Важное);
		Возврат;
	КонецЕсли;
	...
	Если НайденнаяСсылка = Справочники.Номенклатура.ПустаяСсылка() Тогда
		...
		ТекЭлемент1.ГодУрожая = ГодУрожаяАП;
		...
		ОбязательныеПоля.Добавить("ГодУрожая");
		
		ОбязательныеПоляЗаполнены = ПроверкаОбязательныеПоляОбкЗаполнены(ТекЭлемент1, ОбязательныеПоля);
		
		Если НЕ ОбязательныеПоляЗаполнены Тогда
			Сообщить("НЕ заполнены обязательные поля алкоголь доп. свед. для АП с кодом " +  АП.Код, СтатусСообщения.Важное);
			Возврат;
		КонецЕсли;
		...
		Исключение
			Сообщить("Не удалось записать АП с кодом " +  АП.Код, СтатусСообщения.Важное);
			Возврат;
		КонецПопытки;
...
// Функция - Получить год из наименования
//
// Параметры:
//  Наименование - Строка - Строка в составе которой присутствует год (год урожая АП)
// 
// Возвращаемое значение:
//   годКакСтрока - Неопределено если не найдено число из 4х знаков ИЛИ Строка из 4х знаков.
//
Функция ПолучитьГодИзНаименования(знач Наименование)
	
	годКакСтрока 			= Неопределено;	
	стрСодержащаяГод 		= Наименование;	
	разделитель				= НовыйРазделительПоискаЧисла();
	мНаименованияБезЧисел 	= СтрРазделить(стрСодержащаяГод, разделитель, ЛОЖЬ);
	
	стрТолькоДляСимволовЧисла 		= Наименование;	
	Для Каждого толькоТекст Из мНаименованияБезЧисел Цикл
		стрТолькоДляСимволовЧисла = СтрЗаменить(стрТолькоДляСимволовЧисла, толькоТекст, " ");
	КонецЦикла;
	
	мНаименованияТолькоЧисла 	= СтрРазделить(стрТолькоДляСимволовЧисла, " ", Ложь);
	мГруппаИзЧетырехЗнаков		= Новый Массив;
	
	Для Каждого знчЧисло Из мНаименованияТолькоЧисла Цикл
		Если СтрДлина(знчЧисло) <> 4 Тогда
			Продолжить;
		КонецЕсли;
		
		мГруппаИзЧетырехЗнаков.Добавить(знчЧисло);
	КонецЦикла;
	
    годКакСтрока = ?(мГруппаИзЧетырехЗнаков.Количество() <> 1, Неопределено, мГруппаИзЧетырехЗнаков[0]);
	
	Возврат годКакСтрока;
	
КонецФункции // ПолучитьГодИзНаименования()

// Функция - Новый разделитель поиска числа
// 
// Возвращаемое значение:
//   разделитель - Строка - для функции "СтрРазделить"
//
Функция НовыйРазделительПоискаЧисла()

	разделитель			= "";
	наборРазделителей 	= Новый Массив;
	
	наборРазделителей.Добавить("0");
	наборРазделителей.Добавить("1");
	наборРазделителей.Добавить("2");
	наборРазделителей.Добавить("3");
	наборРазделителей.Добавить("4");
	наборРазделителей.Добавить("5");
	наборРазделителей.Добавить("6");
	наборРазделителей.Добавить("7");
	наборРазделителей.Добавить("8");
	наборРазделителей.Добавить("9");
	
	разделитель	= СтрСоединить(наборРазделителей);
	
	Возврат разделитель;
	
КонецФункции // НовыйРазделительПоискаГода()

// Функция - Проверка обязательные поля обк заполнены
//
// Параметры:
//  обк				 - Любой объект с реквизитами 
//  ОбязательныеПоля - Массив Из Строка - Имя реквизита объекта 
// 
// Возвращаемое значение:
//  Булево - Истина или Ложь
//
Функция ПроверкаОбязательныеПоляОбкЗаполнены(обк, ОбязательныеПоля)

	Для Каждого Поле Из ОбязательныеПоля Цикл
		Если НЕ ЗначениеЗаполнено(обк[Поле]) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
КонецФункции // ПроверкаОбязательныеПоляОбкЗаполнены()